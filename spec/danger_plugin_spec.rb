      ...
      describe "linter" do
        ...
        describe :lint_files do
          ...
          it "handles a known JSON report from proselint" do
            @prose.lint_files("spec/fixtures/blog_post.md")
            output = @prose.status_report[:markdowns].first.message
      ...
      describe "spell checking" do
        ...
        describe "full command" do
          ...
          it "handles a known JSON report from mdspell" do
            @prose.check_spelling("spec/fixtures/blog_post.md")
            output = @prose.status_report[:markdowns].first.message
        ...
        it "handles hiding number only spelling results" do
          allow(@prose).to receive(:`).with("which mdspell").and_return("/bin/thing/mdspell")

          proselint_response = "    spec/fixtures/has_numbers_only.md\n        1:21 | Evolved for the next 21 years. \n\n>> 1 spelling error found in 1 file\n"
          allow(@prose).to receive(:`).with("mdspell spec/fixtures/blog_post.md -r").and_return(proselint_response)

          @prose.check_spelling("spec/fixtures/has_numbers_only.md")
          output = @prose.status_report[:markdowns].first.message

          # A title
          expect(output).to_not include("Spell Checker found issues")
          # A typo, in bold
          expect(output).to_not include("**tpyo**")
          # A link to the file inside the fixtures dir
          expect(output).to_not include("[spec/fixtures/has_numbers_only.md](/artsy/eigen/tree/my_fake_branch/spec/fixtures/has_numbers_only.md)")
        end
      end
    end
  end
